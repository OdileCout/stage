...
/*********variables* cadre*******/
    ...
    const insertDocumentation = async()=>{
        ...
        const idDuCadre = ref('')
        const prendreITemraffraichOdrePri = (idCadre, id,raffraichissement,ordreDePriorite) =>{
    ...
    const AttacheDocumentation = async()=>{
        ...
        const insertDocSurCadre = (idCadre, id,raffraichissement,ordreDePriorite) =>{
            prendreITemraffraichOdrePri(idCadre, id,raffraichissement,ordreDePriorite),
...
/** */
    ...
    // insertion dinamique description
...
/** */
    ...
    // fonction qui met ajour le cadre de travail apres suppression de l'icone
     ...
     // fonction pour supprimer une icone
     const iconeDeleted = async(id) =>{
         ...
         const res = await axios(axiosConfig)
        listeIconeCadre.value.findIndex(iconeTodelete =>id == iconeTodelete.id)
    ...
    //fonction qui permet le rafraichissement apres suppression 
...
//la recuperation de données du modal icone
    ...
    const iconeAttacherauCadre = async (id = "") =>{
        try{
            ...
            // permet de change la couleur a l'element attacher
            checkIcon(iconeCadreAttach.value[0].id)
            console.log(iconeCadreAttach.value[0].id);
        }catch(error){
            console.log("On est dans l'erreur")
            console.error(error.message)
        }
...
/* DOCUMENT *******/            
        ...
        const changeDureeAffichDoc = async (iddoc, newDuree) =>{
            try {
        ...
        const updateDureeAffichDoc = async (id, nomdoc) =>{
            await changeDureeAffichDoc(id, nomdoc),
            await getKeyDocs()
        }
        /*** Les fonctions pour la modification de date de début de validité de document *****/ 
        const changeDateDebutDoc = async (iddoc, newDateDV) =>{
            try {
        ...
        const updateDateFVDoc = async (id, nomdoc) =>{
            await changeDateFinDoc(id, nomdoc),
            await getKeyDocs()
        }
        return {
            deleteRefraich,

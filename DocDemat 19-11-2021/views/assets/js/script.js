...
// modaldoc
    ...
    // au démarrage de la page
        try {
            ...
            const res = await axios.get(url)
                res.data.forEach(keyWordDocs => {
                    keyWordDocs.affecter = false
                })
 ...
 // fonction pour faire la recherche dans le tableau
    ...
    //Fonction qui affiche sur le tableau de document à affecté sur un cadre de travail
    const saveDoc = async (id) => {
        try {
            if(valCheck.value !== null){
                ...
                const res = await axios.get(url)
                res.data.forEach(keysDocsSelect => {
                    keysDocsSelect.affected = false
                }) 
                keysDocsSelect.value = res.data
                keyWordDocs.value.forEach(keyWordDocs => {
                    keyWordDocs.affecter = false
                })
            }
        } catch (error) {
...
/*********variables* cadre*******/
    ...
    //desaffecter un document a un cadre de travail
    ...
    // raffrechir la page (dynamique)
    const  insernomCadreandRefraish  = async () => {
        await insernomCadre()
        await cadre()
    }
    //inserer cadre de travail
    ...
    // fonction qui permet de modifier les données si c'est add
    const revenir = async () =>{
        edition.value = !edition.value
        keyWords.value.forEach(keyWords => {
            keyWords.affected=false
        });
        idIconeChange.value=null;
    }
    ...
    const  inserdiapoCadreandRefraish  = async () => {
        await inserDiapoCadre()
        await cadre()
      }
    // inserer les mots cles
    ...
    const effecterMotCle = async(id, idCadreSelected) =>{
        if(idCadreSelected !== null){
            fonctIdMocle(id, idCadreSelected),
            insertMotcle()
        }
    }
    /************** */
    //recuperaton du nom de cadre de travail
    const cadresTravailOptions = computed(() => {
        return cadreDeTravail.value.map(cadre => {
            return {
                label : cadre.nom,
                value : cadre,
                diaporama : cadre.diaporama
            }
        })  
    })
    //recuperation cadre de travail
    const cadre = async () => {
            // au démarrage de la page
        try {
            const url = './../controllers/cadre-de-travail/cadreDeTravailTraitement.php'
            const res = await axios.get(url)
                res.data.forEach(cadreDeTravail => {
                    if(cadreDeTravail.diaporama == 1){
                        cadreDeTravail.diaporama = true
                    }else{
                        cadreDeTravail.diaporama = false 
                    }
                });
            cadreDeTravail.value = res.data
            console.log(cadreDeTravail.value);
        } catch (error) {
            console.log("On est dans l'erreur")
            console.error(error.message)
        }
    ...
    const changeIconAttacher = async (id, idCadreDeTravail) =>{
    ...
    const miseAjourIcones = async () => {
        try {
            ...
            const formData = new FormData()
            formData.append('idIcone', idIconeChange.value)
            formData.append('id',  currentIcone.value) 
            const axiosConfig = {
                method: 'post',
                url: url,
                data: formData,
    ...
    const miseAjourCadrePourIcones = async () => {
        try {
             const url ='./../controllers/cadre-de-travail/miseAjourCadrePourIcon.php'
             const formData = new FormData()
             formData.append('idIcone', idIconeChange.value)
...
/************* TABLEAU MOT CLE *************************/
    ...
    //const qui supprime tout les mots meme attacher a un cadre
    const keyWordAllDeleted  = async (id) =>{
        await keyWordDeletedAll() ,
        await keyWordDeleted(),
        await getKeyWord()
    }

    const keyWordDeletedAll = async () => {
        const url = './../controllers/motCleCadre/keyWordDeletedAllCtrl.php'
        const formData = new FormData()
        formData.append('idMotcle',keyWordTodelete.value)  
        const axiosConfig = {
            method: 'post',
            url: url,
            data: formData,
    ...
    const listeNameIdMotCles = async (id = "") =>{
            try{
                ...
                const res = await axios.get(url)
                res.data.forEach(docNameIdMotCle => {
                    docNameIdMotCle.desaffected=true
                });
...
/************************** TABLEAU ICONE *************************/
    ...
    const messages = ref("")
    const urlImgIcone = "../controllers/iconeCadre/tmp/icones/";
    const urlImgDoc = "../controllers/iconeCadre/tmp/documents/";
...
/**************Pagination accAdmin********************/
    //Fonction qui récupère les cadre de travail attacher à un mot clé
    const text = ref('')
    const listCadreOnIcone = ref([])
    const takeCadreOnMotcle = async()=>{
        try {
            const url = './../controllers/motCleCadre/takeCadreOnMotCleCtrl.php'
            const formData = new FormData();
                formData.append("nomMotcle", searchCadre.value);
                console.log(searchCadre.value);           
                const axiosConfig = {
                    method: 'post',
                    url: url,
                    data: formData,
                ...
                const res =  await axios(axiosConfig)
                listCadreOnIcone.value = res.data;
                searchCadre.value = "";
                text.value = ""
        }catch(error) {
            console.log("On est dans l'erreur")
    ...
    const currentCadres = computed(()=>{
        let IconesMax=([])
        if( listCadreOnIcone.value.length>0){
            const startIndex = cadreByPageFilter.value * (current.value -1)
            let maxIndex = startIndex + cadreByPageFilter.value
            if(maxIndex>listCadreOnIcone.value.length){
                maxIndex=listCadreOnIcone.value.length
            }
            for(let i = startIndex ; i < maxIndex  ; i++){
                IconesMax.push(listCadreOnIcone.value[i])
            }
        }
        return IconesMax   
    })
    //fonction pour faire  la recherche de nom de cadres
    const filteredCadresOfMotcle = computed(() => { 
        const filteredCadreTravail = currentCadres.value.filter((fcadre) => {
            console.log(fcadre);
            return fcadre.nom.toLowerCase().includes(fcadre.nom.toLowerCase());
        })
        console.log(filteredCadreTravail);
        return filteredCadreTravail ;
    })
    const cadreByPageFilter = ref(9)
    const paginationMaxFilter = computed(() => {
        return Math.ceil(listCadreOnIcone.value.length/cadreByPageFilter.value)     
    })
        // const cadres = ref([]) 
    //pagination des cadres de travail
    const current = ref(1)
...
//fonction pour faire  la recherche de nom de cadres par mot cle
...
/****************DOCUMENT *****************/            
            ...
            //Pour mettre le display sur le bouton du type des documents à inserer
            ...
            const typeDoc = (name)=>{
                ...
                if(name == 'documents'){
                    searchInput(name)
                    // deux et trois = display none
                }else if(name === 'video'){
                    searchVideoInput(name)
                }else{
                    searchImageInput(name)
                }
                documents.value = name    
            }
            // const searchVideokey = ref(null)
            const insertDocument = async () => {
                try{
                    ...
                    if(documents.value === 'documents'){
                        ...
                        formData.append("type", documents.value) 
                        console.log(imagefile.files[0]);
                        console.log(documents.value); 
                    }else if(documents.value === 'video'){
                        formData.append("nom",nomNouveauDoc.value)
                        formData.append("url", searchVideokey.value)
                        formData.append("type", documents.value) 
                        console.log(searchVideokey.value);
                        console.log(documents.value);
                    }else{
                        const imagefile = document.querySelector('#UneImage');
                        formData.append("nom",nomNouveauDoc.value)
                        formData.append("chemin", imagefile.files[0])
                        formData.append("type", documents.value) 
                        console.log(imagefile.files[0]);
...
//Liste des document attacher à un cadre de travail
            ...
            const listDocumentAttacherauCadre = async (id) =>{
                try{
                    ...
                    const res = await axios.get(url)
                    res.data.forEach(listeDocumentCadreAttach => {
                        listeDocumentCadreAttach.desaffected = true
                    })
...
//Fonction qui fait appel aux plusieurs fonctions pour l'affichage de la modification
            const relieur = async (id) =>{
                ...
                await getKeyWord(),
                await listDocumentAttacherauCadre(id),
                await listeNameIdMotCles(id),
                await iconeAttacherauCadre(id),
                keysDocsSelect.value = ref([])
            }
...
//supprimer les document sur la corbeil dans le modal docs     
        const deletedToAllDocs = async () => {
            const url = './../controllers/IdcadreEtDocument/deletedToDocsAllCtrl.php'
            const formData = new FormData()
        ...
        const DeleteToDeleteDocs = async(id)=>{
            await deletedToAllDocs()
            await DeletedDocs()
            await getKeyDocs()
        }
        ...
        // fonction qui supprime et rafraichi
        // const deleteRefraichDoc = async () => {
        //     await deletedToAllDocs()
        //     await DeletedDocs()
        //     await getKeyDocs()
        // }
        ...
        //ouvre la boite de dialogue confirmation sup du doc dans le modal
             ...
             recoverNameCadreAndBindId()
             // console.log(deletedTodoc.value);
             }
            
        //afficher le nom des cadres attacher a un document avant suppression 
        const displayResultNameAndDoc = ref('') 
        const recoverNameCadreAndBindId = async () => {
            const url = './../controllers/cadre-de-travail/DisplayNameAndIdDocCtrl.php'
            const formData = new FormData()
            formData.append('id',deletedTodoc.value)
            console.log('id',deletedTodoc.value);
            const axiosConfig = {
                method: 'post',
                url: url,
                data: formData,
        ...
        const res = await axios(axiosConfig)
        displayResultNameAndDoc.value = res.data
        console.log(res.data);
        }
        ...
        const afficherDocuments = async () => { 
                try {
                    ...
                    console.log(docVisionner.value);
                }catch(error){
                    console.error(error.message)
                }
        }
        ...
        const docVisionnerValide = computed(() => {
            return docVisionner.value.filter(doc => new Date(doc.dateDeFinValidite) >= new Date())
        })
...
const supprimerDesDonneesCadre = async(id) =>{
    ...
    const changeValueFab = async()=>{
        fab2.value = true
    }
    const displayResult = ref("")
 
        return {
            filteredCadresOfMotcle,
            takeCadreOnMotcle,
            listCadreOnIcone,
            paginationMaxFilter,
            cadreByPageFilter,
        ...
        //    deleteRefraichDoc, 
           ...
           //delete dans le modal relier a la corbeil
            ...
            columnss,
            keyWordDocs,
         //supp tout les données de la page ajoutModif au click sur la corbeille
            deletedAll, 
            deletedAllDocs,
            deletedAllKeyWord,
            getCadre,

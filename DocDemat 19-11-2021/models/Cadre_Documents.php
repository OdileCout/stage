...
class Cadre_Document extends Database{
    ...
    /**
     * fonction qui delete  tous les docs au check dans la base de données (dans tab doc)
     * @return boolean
     */
    public function deletedAllDocs($idDocs,$idCadre)
    {
     ...
     /**
     * fonction qui delete les iddoc et du cadre attacher dans la base de données
     * @return boolean
     */
    public function deletedToAllDocs(){
        $query = 'DELETE FROM `cadre_Document` WHERE id_document = :id_document';
        $pdoStatment = $this->db->prepare($query);
        $pdoStatment->bindValue('id_document',$this->getIdDocument(), PDO::PARAM_INT);
        return $pdoStatment->execute();
    } 
